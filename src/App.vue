			...
			<section>
				<div class="flex">
					<div class="max-w-xs">
						...
						<div v-if="alreadyTicker" class="text-sm text-red-600">
				...
				<input
					...
					name="wallet"
					id="wallet"
					class="mt-1 block w-100 pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
...
export default {
	...
	created() {
		if (localStorage.length) {
			const localStorageData = JSON.parse(localStorage.getItem('cryptonomiconList'))
			localStorageData.forEach((item, idx) => {
				this.tickers[idx] = {
					name: item.name,
					price: '-'
				}
